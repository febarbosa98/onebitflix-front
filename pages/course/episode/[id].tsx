...
const EpisodePlayer = function () {
    ...
    if(course?.episode === undefined) return <PageSpinner/>
    
    if(episodeOrder + 1 < course?.episode?.length){
        if(Math.round(episodeTime) === course.episode[episodeOrder].secondsLong){
            handleNextEpisode()
        }
    }
    ...
    return (
        <>
        <Head>
            <title>Onebitflix - {course.episode[episodeOrder].name}</title>
        ...
        <main>
            ...
            <Container className="d-flex flex-column align-items-center gap-3 pt-5">
             <p className={styles.episodeTitle}> {course.episode[episodeOrder].name}  </p>

            {typeof window === 'undefined' ? null : (
                <ReactPlayer className={styles.player} url={`${process.env.NEXT_PUBLIC_BASEURL}/episodes/stream?videoUrl=${course.episode[episodeOrder].videoUrl}&token=${sessionStorage.getItem("onebitflix-token")}`} controls
                    ref={playerRef} 
            ...
            <div className={styles.episodeButtonDiv}>
                ...
                <Button className={styles.episodeButton} disabled={episodeOrder + 1 === course.episode.length ? true : false} onClick={handleNextEpisode}>
                    <img src="/episode/iconArrowRight.svg" alt="setaDireita" className={styles.arrowImg} />
                </Button>
            </div>
            <p className="text-center py-4">
                {course.episode[episodeOrder].synopsis}
            </p>
        </Container>
        </main>
